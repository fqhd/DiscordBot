...
module.exports = async function (client) {
	...
	const leaderboardMessage = await channel.messages.fetch("831189816040357898");
	const logMessage = await channel.messages.fetch("852929017945522258");

	await logMessage.edit("Updating Leaderboard...");

	leaderboardString = "";
	leaderboardArray = [];
	leaderboardString += "Congratulations to everyone for ranking up. Summer just started, we all got no life so I don't expect anyone to be travelin... That said, Goodluck and Have fun on the rift!! :)\n\n";
	...
	await leaderboardMessage.edit(leaderboardString);
